...
logging.info('ddddocr验证码识别系统初始化成功')

# 登录页面
URL = 'https://bwgl.zjxkeji.com/user/login'
...
def login_su(url):
    ...
    while True:
        ...
        # 选择下拉框，南宁理工学院
        # 定位下拉框元素
        select_elements = browser.find_elements(By.CSS_SELECTOR, "#schoolId")
...
def handle_play_captcha():
    ...
    try:
        ...
        logging.info(f'识别验证码: {captcha_code}')
...
def main():
    login_su(URL)
    # 调用函数，进行自动看网课，以下为示例
    view_all_courses() # 直接按顺序播放所有课程所有视频
